...
if ((int)$game['player1_id'] === $uid) {
    jsonSuccess(['player_num' => 1, 'game_id' => (int)$game['id'], 'status' => $game['status']]);
}

// Already player 2
if ($game['player2_id'] && (int)$game['player2_id'] === $uid) {
    jsonSuccess(['player_num' => 2, 'game_id' => (int)$game['id'], 'status' => $game['status']]);
}
...
if (!$game['player2_id']) {
    try {
        Database::execute(
            "UPDATE games SET player2_id = ?, status = 'active' WHERE id = ?",
            [$uid, $game['id']]
        );
    } catch (Exception $e) {
        jsonError('Could not join game: ' . $e->getMessage(), 500);
    }
    jsonSuccess(['player_num' => 2, 'game_id' => (int)$game['id'], 'status' => 'active']);
}
